...
public class MatchInfoRepo {
    ...
    public int insert(MatchInfo matchInfo){
        ...
        values.put(MatchInfo.KEY_ScoutType, matchInfo.getScoutType());
    ...
    public void update(MatchInfo matchInfo){
        ...
        db.updateWithOnConflict(MatchInfo.TABLE, values, MatchInfo.KEY_CompId + " = '" + matchInfo.getCompId() + "' AND "
                + MatchInfo.KEY_ScoutType + " = '" + matchInfo.getScoutType() + "'", null, SQLiteDatabase.CONFLICT_IGNORE);
        DatabaseManager.getInstance().closeDatabase();
    }
    ...
    public MatchInfo getMatchInfo(String event, String type){
         ...
         String selectQuery = " SELECT MatchInfo." + MatchInfo.KEY_CurrentMatch
                 ...
                 + " , MatchInfo." + MatchInfo.KEY_ScoutType
                 + " FROM " + MatchInfo.TABLE
                 + " WHERE " + MatchInfo.KEY_CompId + " = '" + event + "'"
                 + " AND " + MatchInfo.KEY_ScoutType + " = '" + type + "'";

        Log.d(TAG, selectQuery);
        Cursor cursor = db.rawQuery(selectQuery, null);

        if(cursor.moveToFirst()){
            matchInfo.setScoutType(cursor.getString(cursor.getColumnIndex(MatchInfo.KEY_ScoutType)));
